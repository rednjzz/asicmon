...
function errWorker() {
    ...
    $.ajax({
        ...
        error: function() {
            // Mark last update time with error flag
            $('#lastUpdated').addClass("error");
            $('err-title').html('FATAL: No response from server');
        },
        complete: function() {
            // Schedule the next request when the current one's complete
...
function lowHashWorker() {
    ...
    function checkLowHash(hash_5s,hash_av, type){
        let targetHash;
        switch(type){
            case 'Antminer S9':
                targetHash = 13000;
        ...
        return ((hash_5s < targetHash * (1 - WARN_TOLERANCE)) && (hash_av < targetHash * (1 - WARN_TOLERANCE)));
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $.each(data.miners, function(index, miner) {
                if (miner !== null) {
                    let warning_class = 'class=warning';

                    if ((!miner.error) && (!miner.offline) && (checkLowHash(miner.hashrate_5s,miner.hashrate_av,miner.miner_type))) {
                        tableContent += '<tr' + warning_class + '>';
                        tableContent += '<td>' + miner.name + '</td>';
                        tableContent += '<td>' + miner.host + '</td>';
                        tableContent += '<td>' + miner.uptime + '</td>';
                        tableContent += '<td>' + miner.miner_type + '</td>';
